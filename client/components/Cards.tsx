import { View, Text, TouchableOpacity, Image } from "react-native";
import React from "react";
import images from "../constants/images";
import icons from "@/constants/icons";

//interface for props
interface Props {
  onPress: () => void;
}

export const FeaturedCard = ({ onPress }: Props) => {
  return (
    <TouchableOpacity
      onPress={onPress}
      className="flex flex-col items-start w-60 relative"
    >
      <Image source={images.japan} className="size-full rounded-t-2xl" />
      <Image
        source={images.cardGradient}
        className="size-full rounded-2xl absolute bottom-0"
      />
      <View className="flex flex-row items-center gap-1 bg-white/90 px-3 py-1.5 rounded-bl-2xl">
        <Image source={icons.star} className="size-3.5" />
        <Text className="text-sm font-rubik-bold text-primary-300">4.4</Text>
      </View>
      <View className="flex flex-col items-start absolute bottom-5 inset-x-5">
        <Text
          className="text-xl font-rubik-extrabold text-white"
          numberOfLines={1}
        >
          Modern Apartment
        </Text>
        <Text className="text-base font-rubik text-white">
          22 W St, New York, NY 10001
        </Text>
      </View>
      <View className="flex flex-row justify-between w-full ">
        <Text className="text-xl font-rubik-extrabold text-white">
          $100,000
        </Text>
        <Image source={icons.heart} className="size-5" />
      </View>
    </TouchableOpacity>
  );
};

export const PopularCard = ({ onPress }: Props) => {
  return (
    <TouchableOpacity
      onPress={onPress}
      className="flex-1 w-full mt-4 px-3 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative"
    >
      <Text>PopularCard</Text>
      <View className="flex flex-col items-start absolute px-3 top-5 right-5 bg-white/90 p-1 rounded-full z-50">
        <Text
          className="text-xl font-rubik-extrabold text-white"
          numberOfLines={1}
        >
          Modern Apartment
        </Text>
        <Text className="text-base font-rubik text-white">
          22 W St, New York, NY 10001
        </Text>
      </View>
      <View className="flex flex-row justify-between w-full ">
        <Text className="text-xl font-rubik-extrabold text-white">
          $100,000
        </Text>
        <Image source={icons.heart} className="size-5" />
      </View>
    </TouchableOpacity>
  );
};
